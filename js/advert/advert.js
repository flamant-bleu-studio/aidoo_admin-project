$(document).ready(function(){
	...
	function jsonLoad()
	{
		if(typeof datasPubEdit !== "undefined" && $("#datas").val() != "") // Si le json existe (voir .tpl)
		{
	...
	$("a.pub_delete").on("click", function(e) {
		e.preventDefault(); // Permet de ne pas revenir en haut de page
	...
	$("#pub_add, #pub_manage .pubs_pub").on("click", function(e){
	...
	$("#valid_pub").on("click", function(){
		...
		if ( $("#formAdvert").validationEngine('validate') )
		{
			...
			if( datas[id]["addtext"] && (datas[id]["addtext"] != "") )
				datas[id]["text"] = tinyMCE.activeEditor.getContent();
			else
				datas[id]["text"] = "";
